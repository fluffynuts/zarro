...
describe(`cache`, () => {
    ...
    describe(`read and write`, () => {
        it(`should write and read within ttl time`, async () => {
            // Arrange
            const sut = cache.create(), key = faker_1.faker.string.alphanumeric(), value = faker_1.faker.number.int();
            // Act
        ...
        it(`should not return an expired value`, async () => {
            // Arrange
            const sut = cache.create(), key = faker_1.faker.string.alphanumeric(), originalValue = 420, fallbackValue = 69;
            // Act
            sut.write(key, originalValue, 0.1);
            await sleep(200);
            const result = sut.read(key, fallbackValue);
            // Assert
            expect(result)
    ...
    describe(`clear`, () => {
        it(`should clear caching`, async () => {
            // Arrange
            const key = faker_1.faker.string.alphanumeric(), value = faker_1.faker.word.sample(), sut = cache.create();
            // Act
            sut.write(key, value, 120);
            expect(sut.read(key))
                .toEqual(value);
    ...
    describe(`through`, () => {
        describe(`synchronous variant`, () => {
            it(`should only call the function once when invoked within ttl`, async () => {
                // Arrange
                let calls = 0;
                const sut = cache.create(), key = faker_1.faker.string.alphanumeric(), expected = faker_1.faker.number.int(), generator = () => {
                    calls++;
                    return expected;
                };
                // Act
        ...
        describe(`async variant`, () => {
            it(`should only call the function once when invoked within ttl`, async () => {
                // Arrange
                let calls = 0;
                const sut = cache.create(), key = faker_1.faker.string.alphanumeric(), expected = faker_1.faker.number.int(), generator = async () => {
                    calls++;
                    await sleep(1);
                    return expected;
                };
