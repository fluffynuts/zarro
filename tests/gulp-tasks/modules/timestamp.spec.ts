...
describe(`timestamp`, () => {
  ...
  describe(`default behavior`, () => {
    it(`should return the timestamp in format yyyyMMddHHmmss`, async () => {
      ...
      const
        now = new Date(),
        expected = `${ z(now.getFullYear()) }${ z(now.getMonth() + 1) }${ z(now.getDate()) }${ z(now.getHours()) }${ z(
          now.getMinutes()) }${ z(now.getSeconds()) }`;
      // Act
  ...
  describe(`options`, () => {
    ...
    describe(`when instructed to use short year`, () => {
      it(`should use short year`, async () => {
        ...
        const
          now = new Date(),
          expected =
            `${ z(now.getFullYear()).substring(2, 4)
            }${ z(now.getMonth() + 1)
            }${ z(now.getDate())
            }${ z(now.getHours())
            }${ z(now.getMinutes())
    ...
    describe(`when given a custom delimiter`, () => {
      it(`should use it`, async () => {
        ...
        const
          now = new Date(),
          delimiter = faker.helpers.arrayElement([ ".", "-", "_", "*" ]),
          expected =
            `${ z(now.getFullYear())
            }${delimiter}${ z(now.getMonth() + 1)
            }${delimiter}${ z(now.getDate())
            }${delimiter}${ z(now.getHours())
            }${delimiter}${ z(now.getMinutes())
            }${delimiter}${ z(now.getSeconds())
            }`;
        // Act
        const result = timestamp({
          forDate: now,
